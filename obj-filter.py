...
def process_pointcloud_file(input_path, output_dir, save_images=True):
    """
    处理单个点云文件：加载、滤波、保存结果
    """
    # 确保输出目录存在
    ...
    output_path = os.path.join(output_dir, filename.replace('.npz', '-fil.npz'))
    ...
    # ========= 2. 可视化滤波前点云 =========
    def plot_pointcloud(xyz, color=None, title="Point Cloud", size=1, save_path=None):
    ...
    if save_images:
        before_image_path = os.path.join(image_dir, f"{filename.replace('.npz', '_before.png')}")
        plot_pointcloud(xyz, color=ch, 
                        title=f"Original Point Cloud: {filename}", 
                        size=1, save_path=before_image_path)
    ...
    # 3.5 基于距离的统计过滤 - 可选步骤
    def distance_filter(values, std_mult=2.5):
        mean = np.mean(values)
        std = np.std(values)
        lower_bound = mean - std_mult * std
        upper_bound = mean + std_mult * std
        return (values >= lower_bound) & (values <= upper_bound)
    
    # 应用距离过滤
    mask_r = distance_filter(r, std_mult=2.5)
    xyz = xyz[mask_r]
    ch = ch[mask_r]
    theta = theta[mask_r]
    ...
    print(f"距离过滤后点数: {xyz.shape[0]}")
    
    # ========= 4. 可视化滤波后点云 =========
    if save_images:
        after_image_path = os.path.join(image_dir, f"{filename.replace('.npz', '_after.png')}")
        plot_pointcloud(xyz, color=ch, 
                        title=f"Filtered Point Cloud: {filename}", 
                        size=2, save_path=after_image_path)
    ...
    print(f"\n处理完成: 原始点数 {original_count} → 滤波后点数 {xyz.shape[0]}")
    print(f"滤波后点云保存至: {output_path}")
    print(f"处理耗时: {time.time()-start_time:.2f}秒")
    
    return output_path

def batch_process_pointclouds(input_dir, output_dir, save_images=True):
    """
    批量处理输入目录中的所有NPZ文件
    """
    # 获取所有npz文件
    ...
    if not npz_files:
        print(f"在目录 {input_dir} 中未找到NPZ文件")
        return
    ...
    for i, file_path in enumerate(npz_files):
        print(f"\n处理文件 {i+1}/{len(npz_files)}")
        result_path = process_pointcloud_file(file_path, output_dir, save_images)
...
if __name__ == "__main__":
    # 配置路径
    input_directory = "cube_pointclouds"  # 包含原始点云NPZ文件的文件夹
    output_directory = "filtered_pointclouds"  # 输出滤波后点云的文件夹
    
    # 执行批量处理
    batch_process_pointclouds(
