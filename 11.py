...
# === 可视化原始点云 ===
def visualize_original_clouds():
    fig, axs = plt.subplots(3, 3, figsize=(15, 15), subplot_kw={'projection': '3d'})
    axs = axs.flatten()

    # 统一坐标范围
    all_points = np.vstack(list(xyz_by_angle.values()))
    pad = 0.1
    limits = {
    ...
    ax.set_title('All Raw Point Clouds')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.view_init(elev=20, azim=-60)
...
def estimate_initial_d():
    ...
    return 0.5 * (np.max(y_90) - np.max(y_270))

# === 配准误差函数 ===
def registration_error(center_xy):
    aligned_points_by_angle = get_aligned_pointclouds(center_xy)
    all_points = np.vstack(list(aligned_points_by_angle.values()))
    centroid = np.mean(all_points, axis=0)
    errors = np.linalg.norm(all_points - centroid, axis=1)
    return np.mean(errors)

# === 初始估计 ===
visualize_original_clouds()
d_init = estimate_initial_d()
x0_init = d_init  # 以 d 作为 x 初始值猜测
initial_guess = np.array([x0_init, 0.0])

# === 执行优化 ===
res = minimize(registration_error, initial_guess, method='Powell')
estimated_center = res.x
